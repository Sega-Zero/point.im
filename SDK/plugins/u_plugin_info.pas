...
type
  ...
  THistNode2 = record
    ...
    HistPath: WideString; //set path to history file here if using qip to write binary history.
                          //if you keep it empty, you should respond to LoadHist method
    Image   : WideString; //Image URI (set to ''  to show standard plugin icon)
  end;
  pHistNode2 = ^THistNode2;
  ...
  IPluginHistory2 = interface(IPluginHistory)
    ['{708C9C05-F9A6-4F10-9A0E-BA49369B9A56}']
    function  AccNodeImg(NodeID: WideString): WideString; stdcall;
  end;
  ...
  IQIPHistory2 = interface(IQIPHistory)
    ['{0521FE0A-555B-4487-B2FF-6713683B96C0}']
    function  AddHistNode2(DllHandle: Integer; ParentNodeID: WideString;
                          NodeInfo: pHistNode2; CanExpand: Boolean): Boolean; stdcall;
  end;
  ...
  TWidgetID = IString;
  TWidgetClass = (wgcMsg, wgcCL);
  TWidgetState = (wgsNormal=1, wgsCompact=2, wgsHot=3, wgsUnused=4, wgsReserved=31);
  TWidgetStateSet = set of TWidgetState;
  TWidgetHintPos = (wpLeft, wpBottom);

  {see http://wiki.qip.ru/SDK Interfaces#IWidget}
  IWidget = interface
  ['{ED8485AF-3828-4FFC-9AD5-9ADB92105167}']
    procedure Click       (MousePt: TPoint); safecall;
    procedure DoubleClick (MousePt: TPoint); safecall;
    procedure MouseDown   (MousePt: TPoint; Button: TMouseButton; ShiftState: TShiftState); safecall;
    procedure MouseUp     (MousePt: TPoint; Button: TMouseButton; ShiftState: TShiftState); safecall;
    procedure MouseMove   (MousePt: TPoint; ShiftState: TShiftState); safecall;
    procedure MouseEnter  (); safecall;
    procedure MouseLeave  (); safecall;

    procedure Draw    (const DC: HDC; DrawRect: TRect; MousePt: TPoint); safecall;
    procedure DrawHint(const DC: HDC; DrawRect: TRect); safecall;

    function  StateAllowed(const CheckState: TWidgetState): Boolean; safecall;

    function  GetPluginHandle(): Integer; safecall;
    function  GetID    (): TWidgetID; safecall;
    function  GetClass (): TWidgetClass; safecall;
    function  IsCompact(): Boolean; safecall; //readonly for plugin
    function  GetState (): TWidgetStateSet; safecall;
    procedure SetState (v: TWidgetStateSet); safecall;
    procedure GetHintInfo(const DC: HDC; var W,H: Integer; var HintPos: TWidgetHintPos); safecall;
  ...
  TPluginBBHint = record
    cbSize    : Integer;    //sizeof(TPluginBBHint)
    ContactID : Integer;    //unique spec contact id
    PluginData: Integer;    //your plugin data pointer (if added)
    ShowRect  : TRect;      //coordinates on screen
    Hint      : WideString; //bb-code
  end;
  pPluginBBHint = ^TPluginBBHint;
...
const
  ...
  PM_PLUGIN_ADD_BTN         = 23; //plugin -> qip, (wParam is pAddBtnInfo - obsolete) LParam is pAddBtnInfo2, core will return Result = Unique Action ID, which plugin will receive on every click on this btn, if Result will be returned  = 0 then btn was not added;
  ...
  PM_PLUGIN_CHAT_ADD_BTN    = 41; //plugin -> qip, (wParam is pAddBtnInfo - obsolete) LParam is pAddBtnInfo2, core will return Result = Unique Action ID, which plugin will receive on every click on this btn, if Result will be returned  = 0 then btn was not added;
  ...
  PM_PLUGIN_NOTIF_ADD_ICON  = 83; //plugin -> qip. Before adding info message, add icon to chat imagelist (to reduce gdi). WParam is HICON, LParam is TPngObject. Result is Icon offset in imagelist NParam is (ImageURI: PWideChar)
  ...
  PM_PLUGIN_GET_QIPHIST     = 90; //plugin -> qip. Request for IQIPHistory2 interface from core. returns in Result
  PM_PLUGIN_GET_PLUGHIST    = 91; //qip -> plugin. Request for IPluginHistory2 interface from plugin. set Result if your plugin supports that interface
  ...
  PM_PLUGIN_READY_FOR_WIDGET   = 114; //qip -> plugin. Notifies plugin that message window has been (re)created and is ready for adding an item
  PM_PLUGIN_WIDGET_ADD         = 115; //plugin -> qip. Instructs core to add plugin item to message window's tab stack. WParam is IWidget, Result is True if successfully added
  PM_PLUGIN_WIDGET_DEL         = 116; //plugin -> qip. Instructs core to delete plugin item to message window's tab stack. WParam is IWidget, Result is True if successfully deleted
  PM_PLUGIN_WIDGET_INVALIDATE  = 117; //plugin -> qip. Request core to invalidate item. WParam is IWidget to be invalidated.
  PM_PLUGIN_WIDGET_SHOW_HINT   = 118; //plugin -> qip. Request core to show hint on specified IWidget. WParam is IWidget, LParam is width of hint, NParam is height of item. Result is True if successfully shown. Note, that hint can be shown only when cursor within the item rect. To close hint, send this message with lparam or nparam = 0. Hint will be closed automatically after mouseleave.
  PM_PLUGIN_WIDGET_INV_HINT    = 119; //plugin -> qip. Request core to invalidate hint on specified IWidget. WParam is IWidget
  PM_PLUGIN_WIDGET_GETBOUNDS   = 120; //plugin -> qip. Request core to get screen bounds on specified IWidget. WParam is IWidget, LParam is pointer to TRect (pRect) for Widget bounds, NParam is pointer to TRect (pRect) for "Widgets" group bounds, Result is True on success
  PM_PLUGIN_HINT_GET_BB        = 121; //qip -> plugin. Get bb-code for hint from plugin. WParam is pPluginBBHint. Plugin must return Result <> 0 if it process this message

  PM_PLUGIN_GETGUI             = 122; //plugin -> qip. [in,out] WParam = address of IQIPCoreGUI instance
  PM_PLUGIN_GET_UTILS          = 123; //plugin -> qip. [in,out] WParam = address of IQIPUtils instance
  PM_PLUGIN_PROTO_STATUS_SET   = 124; //plugin -> qip. Request core to set the status and privacy status for specified proto. WParam is ProtoHandle (may be retrieved from enumeration), LParam is QIP_STATUS_ constant, NParam is QIP_STATUS_PRIV_ (set to 0 if you want to keep current priv status). Result is true if the operation is succeeded
  //to be continued ...
  ...
  QIP_STATUS_NOTINLIST        = $0E; //contacts only
  QIP_STATUS_CONNECTING       = $FF; //protocol only
