...
type
  ...
  obsTAddBtnInfo = record
    obsBtnIcon : HICON;      //size must be 16x16, if you creating new HICON then after adding button dont forget to destroy it in your plugin.
    obsBtnPNG  : LongInt;    //size must be 16x16, LongInt(TPngObject) from PngImage library, delphi only. Else have to be 0. After adding button dont forget to destroy it in your plugin.
  ...
  obspAddBtnInfo = ^obsTAddBtnInfo;

  {see http://wiki.qip.ru/SDK_Types#TAddBtnInfo2 }
  TAddBtnInfo2 = record
    cbSize  : DWORD;
    obsBtnIcon : HICON;      //size must be 16x16, if you creating new HICON then after adding button dont forget to destroy it in your plugin.
    obsBtnPNG  : LongInt;    //size must be 16x16, LongInt(TPngObject) from PngImage library, delphi only. Else have to be 0. After adding button dont forget to destroy it in your plugin.
  ...
  TQipColors = record
    AccBtnText     : integer; //ProtoGroupFont
    AccBtn         : integer; //ProtoGroupGradientTop
    AccBtnGrad     : integer; //ProtoGroupGradientBottom
    AccBtnFrameL   : integer; //ProtoGroupFrameTop
    AccBtnFrameD   : integer; //ProtoGroupFrameBottom

    ContactDelim   : Integer; //sdk 1.12 (rename obsolete online/offline divider "Divider" to "ContactDelim")
    ...
    Focus          : integer; //FocusedGradientTop
    FocusGrad      : integer; //FocusedGradientBottom
    FocusFrame     : integer; //FocusedFrameTop

    Hint           : integer; //HintGradientTop
    HintGrad       : integer; //HintGradientBottom
    HintFrame      : integer; //HintFrameTop
    ...
    obsRootSlctdColor : integer; //sdk 1.12 - obsolete ProtoGroupSelectedFont

    ContactSlctdClr: integer; //FocusedFont
    ...
    ItemBorderColor        : Integer; //fixed tabs delimiter color
    ...
    GroupFrameBottom: Integer;

    //focused++
    FocusedFrameBottom: Integer;
