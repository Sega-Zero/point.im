...
resourcestring
  SCannotOpenClipboard2 = 'Cannot open clipboard (errorcode: %d)';
...
{$ENDIF}       
    ...
    if not OpenClipboard(FClipboardWindow) then
      raise EClipboardError.CreateResFmt(@SCannotOpenClipboard2, [GetLastError]); //EClipboardError в фильтре mad (только чета не работает)
