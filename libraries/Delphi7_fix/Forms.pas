...
uses
  ...
  StdActns{$IFDEF MSWINDOWS}, WinHelpViewer{$ENDIF}, Themes{$IFDEF madExcept}, madExcept{$ENDIF};
...
constructor TScreen.Create(AOwner: TComponent);
  function ReadScreenDPI(): Integer;
  var
    DC: HDC;
    i, dpi, E: Integer;
    s: String;
...
begin
  ...
  begin
    ...
    {$IFDEF madExcept}if HintThread <> 0 then NameThread(ThreadID, 'hint mouse thread');{$ENDIF}
  end;
end;
